...
def repository():
    ...
    mock_repo.upsert_file = MagicMock(
        side_effect=lambda file_metadata: FileMetadataDb(**file_metadata)
    )
    return mock_repo
...
def user_repository():
    ...
    mock_user_repo.get_user = MagicMock(
        return_value=UserDb(
...
def blob_service():
    ...
    mock_blob_service.blob_service_client.get_container_client.return_value = (
...
def test_file_upload_success(
    repository, user_repository, blob_service, test_user, monkeypatch
):
    ...
    monkeypatch.setattr(
        "file_upload.file_upload.QueueService",
        lambda connection_string=None: queue_service,
    )
    ...
    # Create request
    req = MockHttpRequest(method="POST", url="/api/files/upload", params={}, body=None)
...
def test_file_upload_raw_bytes(repository, user_repository, blob_service, test_user):
    ...
    with patch("file_upload.file_upload.QueueService") as mock_queue_class:
        ...
        # Create request
        req = MockHttpRequest(
            method="POST", url="/api/files/upload", params={}, body=None
        )
        req.files = MockFiles({"content": [mock_file]})
        req.form = {"type": "CV"}
        req.headers = {"X-MS-CLIENT-PRINCIPAL": create_mock_b2c_token(test_user.userId)}
...
def test_file_upload_raw_bytes_missing_filename(
    repository, user_repository, blob_service, test_user
):
    # OPTIMIZATION: Use patch instead of monkeypatching
    with patch("file_upload.file_upload.QueueService") as mock_queue_class:
        mock_queue = MagicMock()
...
def test_file_upload_limit_reached(
    repository, user_repository, blob_service, test_user, monkeypatch
):
...
def test_file_upload_user_not_found(
    repository, user_repository, blob_service, monkeypatch
):
    # OPTIMIZATION: Mock queue service
    queue_service = MagicMock()
    monkeypatch.setattr(
...
def test_file_upload_with_content_disposition(
    repository, user_repository, blob_service, test_user, monkeypatch
):
    # OPTIMIZATION: Mock queue service
    queue_service = MagicMock()
...
def test_file_upload_with_form_data_boundary(
    repository, user_repository, blob_service, test_user, monkeypatch
):
    # OPTIMIZATION: Mock queue service
    queue_service = MagicMock()
...
def test_file_upload_without_type(
    repository, user_repository, blob_service, test_user, monkeypatch
):
    # OPTIMIZATION: Use the queue_service fixture instead of monkeypatching
    queue_service = MagicMock()
