...
def sample_file_metadata(repository, blob_service, sample_file_content):
    ...
    try:
        ...
        blob_service.delete_blob(
            container_name="resume-match-pro-files", filename=filename
        )
    except Exception as e:
...
def test_delete_file_integration(repository, blob_service, sample_file_metadata):
    ...
    assert (
        repository.get_file_by_id("test-user-123", str(sample_file_metadata.id)) is None
    )

    # Verify file is deleted from blob storage
    assert not blob_service.blob_exists(
        container_name="resume-match-pro-files", filename=sample_file_metadata.filename
    )
...
def test_delete_file_logic():
    ...
    # Verify calls
    mock_files_repository.get_file_by_id.assert_called_once_with(
        "user-123", str(file_id)
    )
    mock_blob_service.delete_blob.assert_called_once_with(
        container_name="resume-match-pro-files", filename="test.pdf"
    )
    mock_files_repository.delete_file.assert_called_once_with(
        user_id="user-123", file_id=str(file_id)
    )
...
def test_delete_file_logic_missing_params():
    ...
    req1 = func.HttpRequest(
        method="DELETE", url="/api/files", route_params={}, body=None
    )
...
def test_download_file_success(repository, blob_service, sample_file_metadata):
    ...
    assert (
        ...
        == f'attachment; filename="{sample_file_metadata.filename}"'
    )
...
def test_get_file_with_docx_structure(repository, structured_docx_file_metadata):
    ...
    req = func.HttpRequest(
        ...
        headers={
            "X-MS-CLIENT-PRINCIPAL": create_mock_claims(
                structured_docx_file_metadata.user_id
            )
        },
        body=None,
    )
...
def test_get_file_with_pdf_structure(repository, structured_pdf_file_metadata):
    ...
    mock_claims = {
        "claims": [
            {
                "typ": "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier",
                "val": structured_pdf_file_metadata.user_id,
            },
            {"typ": "name", "val": "Test User"},
            {"typ": "emails", "val": "test@example.com"},
        ]
...
@pytest.fixture
def structured_file_metadata(
    repository, blob_service, sample_file_content, sample_resume_structure
):
    # Create a unique filename
    filename = f"test_cv_{uuid4()}.pdf"
    ...
    file_metadata = FileMetadataDb(
        ...
        user_id="test-user-123",
        url=blob_url,
        document_analysis={"document_type": "CV", "structure": sample_resume_structure},
    )
...
def test_get_file_with_structure(repository, blob_service, sample_file_content):
    ...
    assert (
        structure["professional_summary"]
...
def mock_http_request():
    ...
    # Encode to base64
    encoded_principal = base64.b64encode(
...
def test_get_files_with_missing_document_analysis(
    mock_http_request, mock_files_repository
):
    ...
    assert (
        hasattr(file_without_analysis, "document_analysis") == False
        or file_without_analysis.document_analysis is None
    )
...
def test_get_files_with_missing_type(mock_http_request, mock_files_repository):
    ...
    assert (
        hasattr(file_without_type, "document_analysis") == False
        or file_without_type.document_analysis is None
    )
